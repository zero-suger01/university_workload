...
import * as svc from './planning.service';
import { sendSuccess, sendCreated, sendNoContent } from '../../utils/ApiResponse';

export async function list(req: Request, res: Response, next: NextFunction) {
  try {
    const { rows, meta } = await svc.getAll(req.query as Record<string, unknown>, req.user);
    sendSuccess(res, rows, 'Planning rows retrieved', 200, meta);
  } catch (e) { next(e); }
}
...
export async function summary(req: Request, res: Response, next: NextFunction) {
  try {
    sendSuccess(res, await svc.getSummaryBySemester(req.query['semesterId'] as string));
  } catch (e) { next(e); }
}
